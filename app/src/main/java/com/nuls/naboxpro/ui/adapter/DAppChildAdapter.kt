...
class DAppChildAdapter:BaseQuickAdapter<DAppBean,BaseViewHolder> (R.layout.item_dapp_child_layout){
    override fun convert(holder: BaseViewHolder, item: DAppBean) {
        ...
        if(UserDao.language==LanguageEnum.CHS.code){
            ...
            holder.setText(R.id.tv_dapp_desc,item.fileDesc)
        }else{
            holder.setText(R.id.tv_dapp_title,item.fileNameEn)
            holder.setText(R.id.tv_dapp_desc,item.fileDescEn)
        }
